...
deploy_ingress() {
  ...
  # New (used in setting up dynamos with dynamos-configuration.sh, and FABRIC specific):
  helm install -f "${coreChart}/ingress-values.yaml" nginx oci://ghcr.io/nginxinc/charts/nginx-ingress \
    -n ingress --version 0.18.0 \
    --set controller.nodeSelector."kubernetes\\.io/hostname"=dynamos-core
}

# These below again uses the corresponding node by specifying it in the charts folder, just like in the dynamos-configuration.sh script.
